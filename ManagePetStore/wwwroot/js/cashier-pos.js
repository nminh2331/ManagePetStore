...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // [nam] Tải các bảng kê lưu trú đang chờ thu tiền tại quầy POS.
        ...
        try {
            ...
            if (resData.success) {
                ...
                if (resData.hotelCheckouts && resData.hotelCheckouts.length > 0) {
                    hotelHtml = resData.hotelCheckouts.map(hotel => `<div style="border-top:2px dashed #000;margin:15px 0;"></div><div style="border:1px dashed #000;padding:12px;text-align:left;"><div style="font-size:13px;font-weight:bold;text-align:center;margin-bottom:8px;">BẢNG KÊ CHUỒNG HB#${hotel.hotelBookingId}</div><div style="font-size:11px;margin-bottom:6px;">Pet: <strong>${hotel.petName}</strong> · ${hotel.roomType} · chuồng ${hotel.cageId}</div>${(hotel.items || []).map(item => `<div style="display:flex;justify-content:space-between;font-size:11px;"><span>${item.description}</span><strong>${formatCurrency(item.amount)}</strong></div>`).join('')}${Number(hotel.discountAmount || 0) > 0 ? `<div style="display:flex;justify-content:space-between;font-size:11px;"><span>Giảm giá phòng</span><strong>-${formatCurrency(hotel.discountAmount)}</strong></div>` : ''}<div style="display:flex;justify-content:space-between;border-top:1px dashed #000;margin-top:6px;padding-top:5px;"><strong>Tổng chuồng</strong><strong>${formatCurrency(hotel.totalAmount)}</strong></div></div>`).join('');
                }
